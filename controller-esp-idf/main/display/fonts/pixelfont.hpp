...
public:
  ...
  bool has_char(char c) { return get_jumps()[(int)c] != -1; }

  uint8_t get_width(char c) { return get_widths()[(int)c]; };
  ...
  static uint8_t get_undercut(PixelFont &font1, char c1, PixelFont &font2,
                              char c2) {
    if (c1 == ' ' || c2 == ' ') {
      return 0;
    }
